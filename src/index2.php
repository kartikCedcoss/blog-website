...
$numberpage1 = 4;
$page1 = $_GET['start12'];
$start1 = $page1 * $numberpage1;

try {
  $conn = new PDO("mysql:host=$servername;dbname=$blogdb", $username, $password);
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $stmt = $conn->prepare("SELECT * FROM `tb_blog` WHERE tb_blog.userid ='$userid' ORDER BY blogid DESC LIMIT $start1,$numberpage1 ");
  $stmt->execute();
...
<div class="container text-center mt-5" style="margin-left: 50em;">
     <nav aria-label="...">
       <ul class="pagination ">
     <?php
        ...
        try {
            ...
            $stmt->execute();
             $result = $stmt->fetch();
            $numrecord1= $result[0];
            $numlinks1 = ceil($numrecord1/$numberpage1);
            
          } catch(PDOException $e) {
            echo "Error: " . $e->getMessage();
          }
          $conn = null;
         for($i =0; $i<$numlinks1;$i++){
             $y = $i+1;
             echo  '<li class="page-item "><a class="page-link" href="index2.php?start12='.$i.'">'.$y.'</a></li>';
         }
